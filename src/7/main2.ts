...
lines.forEach(line => {
    ...
    const totalJ = groups['J']?.length ?? 0;
    const values = Object.values(groups);
    const totalGroups = totalJ ? values.length - 1 : values.length;
    const maxVal = Math.max(...values.map(items => items[0] === 'J' ? 0 : items.length)) + totalJ;
    ...
    switch(true) {
        ...
        case maxVal === 4:
            ...
            break;
        case maxVal === 3 && totalGroups === 2:
            resultMap['F'].push([cards, amount]);
            break;
        case maxVal === 3 && totalGroups === 3:
            resultMap[3].push([cards, amount]);
            break;
        case maxVal === 2 && totalGroups === 3:
            resultMap[2].push([cards, amount]);
            break;
        case maxVal === 2 && totalGroups === 4:
